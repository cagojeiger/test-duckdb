...
from ..types.core import ExperimentConfig, ExperimentResult
...
class TerminalDashboard:
    ...
    def update_progress(self, completed: int, total: int, current_experiment: Optional[str] = None) -> None:
    ...
    def _create_progress_panel(self) -> Panel:
        ...
        if self.state.total_experiments > 0:
            progress_pct = (self.state.completed_experiments / self.state.total_experiments) * 100
            content_lines.append(f"Experiments: {self.state.completed_experiments}/{self.state.total_experiments} ({progress_pct:.1f}%)")
        else:
        ...
        if self.state.total_batches > 0:
            ...
            content_lines.append(f"Batches: {self.state.current_batch}/{self.state.total_batches} ({batch_pct:.1f}%)")
    ...
    def _create_resources_panel(self) -> Panel:
        ...
        if not self.state.latest_resource_snapshot:
            return Panel("No resource data available", title="Resources", style="yellow")
    ...
    def _create_metrics_panel(self) -> Panel:
        ...
        table.add_row("Insert Time", f"{recent_result.insert_metrics.query_time_ms:.1f} ms")
        table.add_row("Insert QPS", f"{recent_result.insert_metrics.throughput_qps:.1f}")
        table.add_row("Index Time", f"{recent_result.index_metrics.query_time_ms:.1f} ms")
