...
from ..types.dashboard import DashboardState, DashboardUpdate, ExperimentProgress, ResourceMetrics, PerformanceCharts
from ..types.experiment import ExperimentResult
from ..runners.monitoring import ResourceSnapshot


def update_dashboard_state(current_state: DashboardState, update: DashboardUpdate) -> IO[DashboardState]:
    ...
    def _update() -> DashboardState:
        if update.update_type == "progress":
            ...
            new_progress = ExperimentProgress(
                total_experiments=data.get("total", current_state.experiment_progress.total_experiments),
                completed_experiments=data.get("completed", current_state.experiment_progress.completed_experiments),
                failed_experiments=current_state.experiment_progress.failed_experiments,
                current_experiment=data.get("current", current_state.experiment_progress.current_experiment),
...
def apply_dashboard_update(state: DashboardState, update: DashboardUpdate) -> IO[DashboardState]:
    """Apply a dashboard update to the current state"""
    return update_dashboard_state(state, update)


def create_progress_update_effect(completed: int, total: int, current: Optional[str] = None) -> IO[DashboardUpdate]:
...
def render_dashboard_effect(state: DashboardState, render_callback: Callable[[DashboardState], None]) -> IO[None]:
