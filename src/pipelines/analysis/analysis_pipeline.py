...
def load_experiment_results(checkpoint_dir: str) -> IO[List[ExperimentResult]]:
    ...
    def _load_results() -> List[ExperimentResult]:
        ...
        from src.types.core import (
            ExperimentConfig, DataScale, SearchType, FilterConfig, 
            HNSWParams, Metrics, SearchResult, AccuracyMetrics, 
            ExperimentResult, Dimension, Category, Distance
        )
                        ...
                        if 'config' in item and 'results' in item:
                            ...
                            config = ExperimentConfig(
                                ...
                                filter_config=FilterConfig(
                                    ...
                                    category=Category(config_data['filter_category']) if config_data.get('filter_category') else None
                                ),
                                hnsw_params=HNSWParams(
                                    ef_construction=config_data.get('ef_construction', 128),
                            ...
                            insert_metrics = Metrics(
                                ...
                                throughput_qps=metrics_data.get('insert_throughput', 0.0),
                            ...
                            index_metrics = Metrics(
                                query_time_ms=metrics_data.get('index_build_time_ms', 0.0),
                                throughput_qps=0.0,
                                memory_usage_mb=metrics_data.get('memory_usage_mb', 0.0),
                                index_size_mb=metrics_data.get('index_size_mb', 0.0)
                            )
                            ...
                            for search_item in search_data:
                                search_metrics = Metrics(
                                    ...
                                    throughput_qps=search_item.get('throughput_qps', 0.0),
                                    memory_usage_mb=search_item.get('memory_usage_mb', 0.0),
                                ...
                                search_result = SearchResult(
                                    ...
                                    distances=[Distance(d) for d in search_item.get('distances', [])],
...
def generate_visualizations(analysis: PerformanceAnalysis, trends: Optional[TrendAnalysis] = None) -> IO[Dict[str, Any]]:
...
def create_analysis_report(
    ...
    def _create_report() -> str:
        ...
        report_content.append("This report presents a comprehensive analysis of DuckDB Vector Similarity Search (VSS) performance ")
        report_content.append("across different configurations including vector dimensions, data scales, search types, and filter conditions.\n\n")
        ...
        if trends:
            ...
            if trends.temporal_trends:
                ...
                for metric, data_points in trends.temporal_trends.items():
                    report_content.append(f"- **{metric}**: {len(data_points)} measurements over time\n")
                report_content.append("\n")
            
            if trends.scale_trends:
                report_content.append("### Scale Performance Trends\n\n")
                for metric, data_points in trends.scale_trends.items():
                    report_content.append(f"- **{metric}**: Performance scaling across {len(set(dp[0] for dp in data_points))} different data scales\n")
                report_content.append("\n")
            ...
            if trends.dimension_trends:
                ...
                for metric, data_points in trends.dimension_trends.items():
                    report_content.append(f"- **{metric}**: Performance across {len(set(dp[0] for dp in data_points))} different vector dimensions\n")
                report_content.append("\n")
        ...
        if analysis.dimension_performance:
            ...
            report_content.append(f"- **Optimal Vector Dimension**: {best_dim[0]}D shows best performance ({best_dim[1]:.2f}ms avg query time)\n")
            report_content.append(f"- **Performance Impact**: {worst_dim[0]}D takes {worst_dim[1]/best_dim[1]:.1f}x longer than {best_dim[0]}D\n")
        
        if analysis.search_type_comparison:
            best_search = min(analysis.search_type_comparison.items(), key=lambda x: x[1])
            report_content.append(f"- **Recommended Search Type**: {best_search[0]} provides best performance ({best_search[1]:.2f}ms avg query time)\n")
        
        if analysis.filter_impact and 'impact_ratio' in analysis.filter_impact:
            impact_ratio = analysis.filter_impact['impact_ratio']
            if impact_ratio > 1.5:
                report_content.append(f"- **Filter Optimization Needed**: Filters add {impact_ratio:.1f}x overhead to query time\n")
            else:
                report_content.append(f"- **Filter Performance**: Acceptable {impact_ratio:.1f}x overhead from filtering\n")
