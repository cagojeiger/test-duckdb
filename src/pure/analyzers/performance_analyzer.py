...
def analyze_dimension_performance(results: List[ExperimentResult]) -> PerformanceAnalysis:
    ...
    if filtered_results and unfiltered_results:
        ...
        if filtered_times and unfiltered_times:
            filter_impact = {
                ...
                "impact_ratio": statistics.mean(filtered_times) / statistics.mean(unfiltered_times)
            }
...
def analyze_search_type_performance(results: List[ExperimentResult]) -> Dict[str, float]:
...
def compare_accuracy_metrics(results: List[ExperimentResult]) -> Dict[str, Dict[str, float]]:
    ...
    for result in results:
        ...
        search_type_accuracy[search_type]["recall_at_1"].append(result.accuracy.recall_at_1)
        search_type_accuracy[search_type]["recall_at_5"].append(result.accuracy.recall_at_5)
        search_type_accuracy[search_type]["recall_at_10"].append(result.accuracy.recall_at_10)
        search_type_accuracy[search_type]["mrr"].append(result.accuracy.mean_reciprocal_rank)
    
    for search_type, metrics in search_type_accuracy.items():
        accuracy_comparison[search_type] = {
            "avg_recall_at_1": statistics.mean(metrics["recall_at_1"]) if metrics["recall_at_1"] else 0.0,
            "avg_recall_at_5": statistics.mean(metrics["recall_at_5"]) if metrics["recall_at_5"] else 0.0,
            "avg_recall_at_10": statistics.mean(metrics["recall_at_10"]) if metrics["recall_at_10"] else 0.0,
