...
class TestPerformanceAnalyzer:
    ...
    def create_sample_experiment_result(
        ...
        config = ExperimentConfig(
            ...
            hnsw_params=HNSWParams(
                ef_construction=128, ef_search=64, M=16, metric="cosine"
            ),
            batch_size=1000,
            num_queries=100,
        )
    ...
    def test_analyze_dimension_performance_single_result(self) -> None:
        ...
        result = self.create_sample_experiment_result(
            dimension=Dimension(256), query_time_ms=15.0
        )
        results = [result]
    ...
    def test_analyze_dimension_performance_multiple_dimensions(self) -> None:
        """여러 차원에 대한 성능 분석"""
        results = [
            self.create_sample_experiment_result(
                dimension=Dimension(128), query_time_ms=10.0
            ),
            self.create_sample_experiment_result(
                dimension=Dimension(256), query_time_ms=20.0
            ),
            self.create_sample_experiment_result(
                dimension=Dimension(512), query_time_ms=30.0
            ),
        ]
    ...
    def test_analyze_dimension_performance_filter_impact(self) -> None:
        ...
        results = [
            self.create_sample_experiment_result(
                filter_enabled=False, query_time_ms=10.0
            ),
            self.create_sample_experiment_result(
                filter_enabled=True, query_time_ms=15.0
            ),
        ]
        ...
        assert (
            analysis.filter_impact["with_filter"]
            > analysis.filter_impact["without_filter"]
        )
    ...
    def test_analyze_search_type_performance_multiple_types(self) -> None:
        ...
        results = [
            self.create_sample_experiment_result(
                search_type=SearchType.PURE_VECTOR, query_time_ms=10.0
            ),
            self.create_sample_experiment_result(
                search_type=SearchType.HYBRID, query_time_ms=15.0
            ),
        ]
    ...
    def test_compare_accuracy_metrics_multiple_search_types(self) -> None:
        ...
        result1 = self.create_sample_experiment_result(
        ...
        assert (
            comparison["hybrid"]["avg_recall_at_1"]
            > comparison["pure_vector"]["avg_recall_at_1"]
        )
        assert (
            comparison["hybrid"]["avg_recall_at_5"]
            > comparison["pure_vector"]["avg_recall_at_5"]
        )
        assert (
            comparison["hybrid"]["avg_recall_at_10"]
            > comparison["pure_vector"]["avg_recall_at_10"]
        )
